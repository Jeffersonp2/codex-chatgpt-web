...
if (-not (Test-Path (Join-Path $Here 'node_modules'))) {
    Write-Host 'Instalando dependencias do modulo de desenvolvimento TEAMSIX...' -ForegroundColor Cyan
    bun install
}
...
Write-Host ''
Write-Host '=== TEAMSIX AI BRIDGE - MODO DESENVOLVIMENTO ===' -ForegroundColor Yellow
Write-Host 'Para uso normal, execute o aplicativo empacotado TEAMSIX AI Bridge.' -ForegroundColor DarkGray
...
Write-Host "Upstream debug: $($env:TEAMSIX_CHATGPT_WEB_URL)" -ForegroundColor DarkGray
Write-Host ''
